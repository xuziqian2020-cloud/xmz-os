  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (isXiaomeiLogin) {
        ...
        if (!res.ok || !data.ok) {
          setError(data.error || "登录失败，请检查配置。")
          return
        }
    ...
    <main className="flex min-h-[100dvh] items-center justify-center bg-slate-950 px-6 py-10 text-white">
      <section className="w-full max-w-[440px]">
        <div className="mb-7 flex items-center gap-3">
          <div className="h-14 w-14 overflow-hidden rounded-full border border-slate-700 bg-slate-900">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-7 shadow-2xl shadow-black/30">
          ...
          <form className="mt-7 space-y-5" onSubmit={handleSubmit}>
            ...
            >
              {loading ? "正在处理..." : isXiaomeiLogin || demoMode ? "进入工作台" : "登录或首次注册"}
